...
def build_readiness_profile(files: list[dict], business_profile: dict | None = None, models: dict | None = None) -> dict:
    ...
    if score >= 80:
    ...
    elif has_tb and (has_revenue or has_expenses):
        label = "نطاق تحليل مالي جيد"
        missing_parts = []
        if not has_cash:
            missing_parts.append("حركة النقد والـ Runway")
        if not has_ar:
        ...
        status = f"ميزان المراجعة يكفي لبناء قائمة الدخل والمركز المالي والنسب الأساسية. الملفات التفصيلية المرفوعة تضيف تحليلًا شهريًا ومقارنة، وما زال تحليل {missing_txt} يحتاج ملفًا إضافيًا لرفع الدقة."
    elif has_tb:
        label = "نطاق Financial Analysis أساسي"
        status = "ميزان المراجعة وحده كافٍ لاستخراج قائمة الدخل، المركز المالي، نسب الربحية والسيولة والمديونية الأساسية. ملفات المبيعات والمصاريف والسيولة ليست شرطًا للبدء؛ لكنها تضيف التوزيع الشهري والتحصيل والتنبؤ."
    elif has_revenue and has_expenses:
        label = "نطاق قراءة تشغيلية مبدئية"
        status = "يمكن بناء قراءة أولية للإيراد والمصاريف والربح التشغيلي من الملفات المرفوعة، لكن المركز المالي والسيولة المحاسبية ونسب المديونية تحتاج ميزان مراجعة."
    elif has_revenue or has_expenses:
        label = "نطاق جزئي"
        status = "يوجد ملف مالي واحد فقط، لذلك سنعرض ما يمكن قراءته منه دون الادعاء بتشخيص كامل. ارفع ميزان مراجعة أو الملف المكمل لبدء نموذج قابل للاعتماد."
    else:
        label = "لا توجد بيانات مالية كافية"
        status = "ابدئي برفع ميزان المراجعة. هو الحد الأدنى لبناء القوائم والنسب، ثم تضيف الملفات الأخرى عمقًا لا شرطًا لإيقاف التحليل."
...
def build_missing_data_recommendations(profile: dict) -> pd.DataFrame:
    ...
    if "تفاصيل إضافية للتنبؤ" in missing:
