...
def build_ratio_insights(pnl_model: dict, ratio_model: dict | None = None) -> dict:
    ...
    if net_margin >= 0.20 and gross_margin >= 0.50 and opex_ratio <= 0.50:
        ...
        decision = "الإجراء المقترح: تثبيت نموذج الربحية الحالي، ومراقبة أكبر بنود المصاريف والتحصيل قبل اعتماد التزام مالي جديد."
        ...
        decision = "الإجراء المقترح: إيقاف القرارات المالية الجديدة مؤقتاً إلى حين مراجعة التسعير والتكاليف الثابتة والمتغيرة."
...
def build_breakeven_insights(pnl_model: dict, breakeven_model: dict) -> dict:
    ...
    if be <= 0 or contribution_margin <= 0:
        ...
        decision = "الإجراء المقترح: رفع الإيراد المتكرر أو تخفيض التكاليف الثابتة قبل الالتزام بالتزام مالي جديد."
    else:
        ...
        decision = "الإجراء المقترح: تحديد فجوة الإيراد المطلوبة وخطة تخفيض تكاليف قبل أي قرار التزام مالي جديد."
...
def build_forecast_insights(forecast_df: pd.DataFrame, pnl_model: dict) -> dict:
    ...
    if avg_base_profit > 0 and avg_base_margin >= 0.10 and min_all_profit >= 0:
        ...
        decision = "الإجراء المقترح: عدم اعتماد أي التزام مالي جديد إلا بعد تحديد حد أدنى للإيراد الشهري وحد أقصى للمصاريف."
    else:
        status = "توقعات مقلقة"
        risk = "السيناريو الأساسي يشير إلى ربحية ضعيفة أو سلبية."
        decision = "الإجراء المقترح: مراجعة التسعير والتكاليف قبل اتخاذ أي قرار التزام مالي جديد."
...
def build_forecast_assumptions_table():
    return pd.DataFrame([
        ...
        ["أساسي", "Base", "استمرار متوسط الأداء الحالي", "ثبات نسبي في المصاريف", "قياس استمرار الوضع الحالي دون التزام مالي جديد"],
        ["نمو", "Growth", "زيادة الإيراد فوق المتوسط الحالي", "ارتفاع محدود في المصاريف", "اختبار أثر الالتزام مالي جديد المنضبط"],
