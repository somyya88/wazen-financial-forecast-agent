...
def build_diagnostic_findings(metric_pack: dict, profile: dict | None = None) -> pd.DataFrame:
    ...
    if gm is not None and om is not None:
        if gm > 0.35 and om < 0.05:
            ...
            add("نموذج العمل", "المشكلة تبدأ من اقتصاد الخدمة أو المنتج، وليس من المصاريف الإدارية فقط.", f"Gross Margin {_pct(gm)} و Operating Margin {_pct(om)}", "خطر", "تكلفة الإيراد عالية أو التسعير ضعيف أو هناك خدمات/منتجات بهامش منخفض.", "خفض المصاريف الإدارية وحده لن يعالج الخسارة إذا بقي الهامش الإجمالي ضعيفًا.", "ابدأ بتسعير الباقات/الخدمات أو تكلفة الصنف/الخدمة قبل أي التزام مالي جديد.", "Gross Margin by service/product", 100)
    ...
    if sm is not None and sm > 0.25:
        add("النمو", "تكلفة البيع والتسويق مرتفعة وتحتاج ربطها بنتائج فعلية.", f"S&M / Revenue {_pct(sm)}", "متوسط", "حملات أو عمولات أو تكلفة مبيعات غير مرتبطة بوضوح بعائد قابل للقياس.", "قد ترتفع المبيعات ظاهريًا بينما يتآكل الهامش بسبب تكلفة النمو.", "اربط مصاريف التسويق والعملاء الجدد وصافي الإيراد، ولا التزام مالي جديد الإنفاق قبل معرفة العائد.", "S&M % و Revenue Growth", 70)
    ...
    if ocf is not None and ocf < 0.6:
        add("جودة الربح", "الربح لا يتحول إلى نقد بالقدر الكافي.", f"OCF / Net Income {_pct(ocf)}", "خطر", "الربح قد يكون محبوسًا في الذمم أو متأثرًا بحركات غير نقدية أو تأخر التحصيل.", "الأرباح الدفترية لا تحمي الشركة إذا لم تتحول إلى نقد تشغيلي.", "قارن صافي الربح بالتدفق النقدي التشغيلي وبأعمار العملاء قبل اعتماد أي التزام مالي جديد.", "OCF / Net Income", 90)
...
def build_ratio_narratives(metric_pack: dict, findings: pd.DataFrame | None = None, profile: dict | None = None) -> pd.DataFrame:
    ...
    def narrative_for(row):
        ...
        if key == "revenue_leakage_ratio":
            ...
            action = "افصل تكلفة الإيراد حسب خدمة/صنف/عميل، وراجع الأسعار أو تكلفة تقديم الخدمة قبل أي التزام مالي جديد."
            ...
            cause = "ارتفاعها قد يعني زيادة إدارية سابقًا لنمو الإيراد، أو عقودًا ثابتة لا تتغير مع حجم المبيعات."
            ...
            action = "قارن صافي الربح بتقرير السيولة وأعمار العملاء قبل اعتماد أي التزام مالي جديد."
