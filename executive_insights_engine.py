...
def build_financial_performance_scorecard(pnl_model: dict, breakeven_model: dict | None = None, data_quality: dict | None = None):
    ...
    if net_margin >= 0.20 and opex_ratio <= 0.45 and margin_of_safety >= 0.30:
        ...
        action = "تثبيت نموذج الربحية، ومراقبة تكلفة الإيراد والمصاريف التشغيلية قبل أي التزام مالي جديد."
        ...
        action = "وضع حد شهري للمصاريف، ومراجعة البنود الأكبر قبل اعتماد أي التزام مالي جديد."
    else:
        status = "أداء يحتاج تدخل"
        risk = "الهامش أو هامش الأمان لا يوفران مساحة كافية لتحمل انخفاض الإيرادات أو ارتفاع المصاريف."
        action = "مراجعة التسعير والتكاليف الثابتة والمتغيرة قبل القرارات المالية الجديدة."
...
def build_forecast_decision(forecast_df: pd.DataFrame, breakeven_model: dict):
    ...
    if worst_profit < 0:
        ...
        decision = f"عدم اعتماد أي التزام مالي جديد قبل ضمان إيراد شهري أعلى من {safe_revenue_monthly:,.0f} وضبط سقف المصاريف."
    else:
