...
def build_decision_action_plan(pnl_model, breakeven_model=None, expense_model=None, revenue_monthly=None):
    ...
    rows.append({
        "الأولوية": 3,
        "الموضوع": "تفسير تذبذب الإيرادات قبل أي التزام مالي جديد",
    ...
    rows.append({
        ...
        "القرار المطلوب": "لا يتم اعتماد التزام مالي جديد أو زيادة رواتب قبل اختبار أثر القرار على السيولة الأسبوعية.",
...
def build_owner_ratios_summary(pnl_model, breakeven_model=None, expense_model=None):
    ...
    if m["net_margin"] >= 0.12 and m["opex_ratio"] <= 0.40:
        ...
        action = "ابدأ ببناء توقع سيولة قبل أي التزام مالي جديد."
...
def build_revenue_decision_table(revenue_monthly):
    ...
    return pd.DataFrame([
        ...
        {"المؤشر": "متوسط الإيراد الشهري", "القيمة": money(avg), "القراءة": "يستخدم كخط أساس محافظ، وليس كضمان لأي التزام مالي جديد."},
    ])
